...
import trimesh
@register_env("TwoStep_my_together", max_episode_steps=50)
class TwoStep_my_together_Env(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(self, *args, robot_uids="panda", robot_init_qpos_noise=0.02, **kwargs):
        ...
        self.all_relation_cube_out_bin = ["blue block is inside the white box",
                        "blue block is next to the white box",
                        "blue block is above the white box, grasped by robot hand"]
        self.all_embedding_relation_cube_out_bin = []
        for sentence in self.all_relation_cube_out_bin:
            text_embedding = self.encode_text(sentence)
            text_embedding = text_embedding / text_embedding.norm(dim=-1, keepdim=True)
            self.all_embedding_relation_cube_out_bin.append(text_embedding[0].cpu().numpy())
        ...
        self.all_relation_hand_cube_out = ["robot hand is grasping the blue block",
                                 "robot hand is not grasping the blue block"]
        self.all_embedding_relation_hand_cube_out = []
        for sentence in self.all_relation_hand_cube_out:
            text_embedding = self.encode_text(sentence)
            text_embedding = text_embedding / text_embedding.norm(dim=-1, keepdim=True)
            self.all_embedding_relation_hand_cube_out.append(text_embedding[0].cpu().numpy())
    ...
    def _load_scene(self, options: dict):
        ...
        # load the cube
        self.cube_out = actors.build_cube(
            self.scene,
            half_size=self.cube_half_size*0.6,
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            xyz[..., 2] = self.cube_half_size*0.6  # on the table
            q = [1, 0, 0, 0]
            obj_pose = Pose.create_from_pq(p=xyz, q=q)
            self.cube_out.set_pose(obj_pose)
    ...
    def evaluate(self):
        ...
        is_obj_static = self.cube_out.is_static(lin_thresh=1e-2, ang_thresh=0.5)
        is_obj_grasped = self.agent.is_grasping(self.cube_out)
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        pos_cube_out = self.cube_out.pose.p
        pos_bin = self.bin.pose.p
        offset = pos_cube_out - pos_bin
        xy_flag = torch.linalg.norm(offset[..., :2], axis=1) <= 0.005
        z_flag = (
            torch.abs(offset[..., 2] - self.cube_half_size*0.6 - self.block_half_size[0]) <= 0.005
        )
        is_cube_out_on_bin = torch.logical_and(xy_flag, z_flag)
        is_cube_out_static = self.cube_out.is_static(lin_thresh=1e-2, ang_thresh=0.5)
        is_cube_out_grasped = self.agent.is_grasping(self.cube_out)



        if is_cube_out_on_bin:
            relation_cube_out_bin_semantics = [0]
            relation_cube_out_bin_embedding = [self.all_embedding_relation_cube_out_bin[0]]
            out_bin_done = True
        elif is_cube_out_static and (not is_cube_out_on_bin):
            relation_cube_out_bin_semantics = [1]
            relation_cube_out_bin_embedding = [self.all_embedding_relation_cube_out_bin[1]]
            out_bin_done = False
        elif xy_flag and is_cube_out_grasped and (not z_flag):
            relation_cube_out_bin_semantics = [2]
            relation_cube_out_bin_embedding = [self.all_embedding_relation_cube_out_bin[2]]
            out_bin_done = False
        else:
            relation_cube_out_bin_semantics = [2]
            relation_cube_out_bin_embedding = [self.all_embedding_relation_cube_out_bin[2]]
        ...
        if is_cube_out_grasped:
            relation_hand_cube_out_semantics = [0]
            relation_hand_cube_out_embedding = [self.all_embedding_relation_hand_cube_out[0]]
        else:
            relation_hand_cube_out_semantics = [1]
            relation_hand_cube_out_embedding = [self.all_embedding_relation_hand_cube_out[1]]
        ...
        obs = {
            ...
            "relationship_cube_out_bin_semantics": relation_cube_out_bin_semantics,
            "relationship_hand_cube_out_semantics": relation_hand_cube_out_semantics,
            ...
            "relationship_cube_out_bin_embedding": relation_cube_out_bin_embedding,
            "relationship_hand_cube_out_embedding": relation_hand_cube_out_embedding,
            
            "agent_tcp_pose": self.agent.tcp.pose.raw_pose,
            
            "in_bin_done": in_bin_done,
            "out_bin_done": out_bin_done,
            
            "is_cube_in_on_bin": is_cube_in_on_bin,
            "is_cube_out_on_bin": is_cube_out_on_bin,
