...
class TableSortOneactionExp1Type1Env(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        builder_2 = self._load_model("005_tomato_soup_can")
        ...
        self.obj_2 = builder_2.build(name="obj_2_can")
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        if True:  # torch.device ctx not supported in PyTorch < 2.0
            env_b = len(env_idx)
            self.scene_builder.initialize(env_idx)

            #apple, banana, cube
            obj_p_choices = [torch.tensor([-0.1, -0.2]+(2*np.random.rand(2)-1)*0.01), torch.tensor([-0.01, 0.06]+(2*np.random.rand(2)-1)*0.01), torch.tensor([-0.05, -0.05]+(2*np.random.rand(2)-1)*0.01)]
            position_pick = [0, 1, 2]
            ...
            self.obj_2.set_pose(Pose.create_from_pq(torch.concat((obj_p_choices[position_pick[2]], torch.tensor([0.051011271000000004]))), torch.tensor([[0.1118, 0.0000, 0.0000, 0.9937]])))
...
@register_env(
    "TableSortOneactionExp1Type1",
    asset_download_ids=["ycb", "pick_clutter_ycb_configs"],
    max_episode_steps=100,
)
class TableSortOneactionExp1Type1Env(TableSortOneactionExp1Type1Env):
