...
from mani_skill.envs.tasks.tabletop.table_sort_my_oneaction_exp1_type2 import TableSortOneactionExp1Type2Env
from mani_skill.examples.motionplanning.panda.motionplanner import \
    PandaArmMotionPlanningSolver
from mani_skill.examples.motionplanning.panda.utils import (
    compute_grasp_info_by_obb, get_actor_obb)

def solve(env: TableSortOneactionExp1Type2Env, seed=None, debug=False, vis=False):
    env.reset(seed=seed)
    env.reset_agent()
    ...
    # Reach
    # -------------------------------------------------------------------------- #
    reach_pose = grasp_pose * sapien.Pose([0, 0, -0.07])
    ...
    # Move to goal pose
    # -------------------------------------------------------------------------- #
    goal_pose = sapien.Pose(p=env.goal_region_0.pose.sp.p + np.array([0.02, 0.05, 0.1]),q=env.agent.tcp.pose.sp.q)
    ...
    # # Lift
    # # -------------------------------------------------------------------------- #
    lift_pose = sapien.Pose([0, 0, 0.06]) * goal_pose
    ...
    # Move to goal pose
    # -------------------------------------------------------------------------- #
    goal_pose = sapien.Pose(p=env.goal_region_1.pose.sp.p + np.array([0.02, -0.05, 0.1]),q=env.agent.tcp.pose.sp.q)
    planner.move_to_pose_with_screw(goal_pose)
