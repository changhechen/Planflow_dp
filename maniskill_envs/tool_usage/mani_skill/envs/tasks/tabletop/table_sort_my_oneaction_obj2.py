...
class TableSortOneactionObj2Env(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.all_relation_obj_bin = ["blue block is on target place",
                        "blue block is next to target place",
                        "blue block is above target place, grasped by robot hand"]
        ...
        self.all_relation_hand_obj = ["robot hand is grasping the blue block",
    ...
    def _load_scene(self, options: dict):
        ...
        #013_apple
        #builder_0 = self._load_model("030_fork")
        #builder_0 = self._load_model("025_mug")
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            item_range_0 = [[-0.07, 0.03], [0.02, 0.07]]
            
            item_range_1 = [[-0.07, -0.02], [0.02, 0.01]]
            
            item_range_2 = [[-0.07, -0.07], [0.02, -0.05]]
            
            

            
            obj_p_choices = [torch.tensor([-0.1, -0.2]), torch.tensor([0, 0.03]), torch.tensor([-0.05, -0.05])]
            position_pick = [0, 1, 2]
            np.random.shuffle(position_pick)
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        pos_obj = self.obj_2.pose.p
        pos_bin = self.goal_region_0.pose.p
        offset = pos_obj - pos_bin
        xy_flag = torch.linalg.norm(offset[..., :2], axis=1) <= 0.02
        z_flag = (
            torch.abs(offset[..., 2] - 0.02) <= 1e-3
        )
        
        is_obj_on_bin = torch.logical_and(xy_flag, z_flag)
        is_obj_static = self.obj_2.is_static(lin_thresh=1e-2, ang_thresh=0.5)
        is_obj_grasped = self.agent.is_grasping(self.obj_2)
...
@register_env(
    "TableSortMyOneactionObj2",
    asset_download_ids=["ycb", "pick_clutter_ycb_configs"],
    max_episode_steps=100,
)
class TableSortMyOneactionObj2Env(TableSortOneactionObj2Env):
