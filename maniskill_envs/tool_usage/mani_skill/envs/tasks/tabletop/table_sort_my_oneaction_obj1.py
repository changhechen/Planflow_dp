...
class TableSortOneactionObj1Env(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.all_relation_obj_bin = ["banana is on target place",
                        "banana is next to target place",
                        "banana is above target place, grasped by robot hand"]
        ...
        self.all_relation_hand_obj = ["robot hand is grasping the banana",
                                 "robot hand is not grasping the banana"]
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        pos_obj = self.obj_1.pose.p
        pos_bin = self.goal_region_0.pose.p
        offset = pos_obj - pos_bin
        xy_flag = torch.linalg.norm(offset[..., :2], axis=1) <= 0.02
        z_flag = (
            torch.abs(offset[..., 2] - 0.01824372996084004) <= 1e-3
        )
        
        is_obj_on_bin = torch.logical_and(xy_flag, z_flag)
        is_obj_static = self.obj_1.is_static(lin_thresh=1e-2, ang_thresh=0.5)
        is_obj_grasped = self.agent.is_grasping(self.obj_1)
...
@register_env(
    "TableSortMyOneactionObj1",
    asset_download_ids=["ycb", "pick_clutter_ycb_configs"],
    max_episode_steps=100,
)
class TableSortMyOneactionObj1Env(TableSortOneactionObj1Env):
