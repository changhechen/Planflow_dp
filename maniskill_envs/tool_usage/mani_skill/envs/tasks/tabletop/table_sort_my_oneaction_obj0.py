...
class TableSortOneactionObj0Env(BaseEnv):
    ...
    def _get_obs_extra(self, info: Dict):
        ...
        pos_obj = self.obj_0.pose.p
        pos_bin = self.goal_region_0.pose.p
        offset = pos_obj - pos_bin
        xy_flag = torch.linalg.norm(offset[..., :2], axis=1) <= 0.02
        z_flag = (
            torch.abs(offset[..., 2] - 0.035551499) <= 1e-3
        )
        
        is_obj_on_bin = torch.logical_and(xy_flag, z_flag)
        is_obj_static = self.obj_0.is_static(lin_thresh=1e-2, ang_thresh=0.5)
        is_obj_grasped = self.agent.is_grasping(self.obj_0)
...
@register_env(
    "TableSortMyOneactionObj0",
    asset_download_ids=["ycb", "pick_clutter_ycb_configs"],
    max_episode_steps=100,
)
class TableSortMyOneactionObj0Env(TableSortOneactionObj0Env):
