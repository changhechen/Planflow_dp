...
def build_my_target(
    ...
    scene_idxs: Optional[Array] = None,
    initial_pose: Optional[Union[Pose, sapien.Pose]] = None,
    target_color = np.array([194, 19, 22, 255]) / 255
):
    builder = scene.create_actor_builder()
    builder.add_cylinder_visual(
        radius=radius,
        half_length=thickness / 2,
        material=sapien.render.RenderMaterial(base_color=target_color),
    )
...
def build_blue_target(
    ...
    TARGET_BLUE = np.array([0, 0, 128, 255]) / 255
    builder = scene.create_actor_builder()
    builder.add_cylinder_visual(
        radius=radius,
        half_length=thickness / 2,
        material=sapien.render.RenderMaterial(base_color=TARGET_BLUE),
    )
...
def build_yellow_target(
    ...
    TARGET_YELLOW = np.array([255, 204, 0, 255]) / 255
    builder = scene.create_actor_builder()
    builder.add_cylinder_visual(
        radius=radius,
        half_length=thickness / 2,
        material=sapien.render.RenderMaterial(base_color=TARGET_YELLOW),
    )
