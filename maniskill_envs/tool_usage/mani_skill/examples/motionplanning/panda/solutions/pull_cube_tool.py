...
def solve(env: PullCubeToolEnv, seed=None, debug=False, vis=False):
    ...
    offset = sapien.Pose([0.02+0.02, 0, 0])
    ...
    approach_offset = sapien.Pose(
        [-(env.hook_length + env.cube_half_size + 0.08)+0.02,  
        -0.0,  
        lift_height - 0.05]  
    )
    ...
    #____________second operation______
    
    # -------------------------------------------------------------------------- #
    # Remove the tool
    # -------------------------------------------------------------------------- #
    remove_offset = sapien.Pose([0, 0.1, 0])
    target_pose = target_pose * remove_offset
    res = planner.move_to_pose_with_screw(target_pose)
    if res == -1: return res
    ...
    # -------------------------------------------------------------------------- #
    lift_height = 0.1 
    lift_pose = sapien.Pose(target_pose.p + np.array([0, 0, lift_height]))
    lift_pose.set_q(target_pose.q)  # Maintain grasp orientation
    res = planner.move_to_pose_with_screw(lift_pose)
    if res == -1: return res
    ...
    offset = (goal_pose.p - env.cube.pose.p).numpy()[0] # remember that all data in ManiSkill is batched and a torch tensor
