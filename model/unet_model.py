...
def get_vit(name:str="vit_b_16", weights=None, **kwargs) -> nn.Module:
    func = getattr(torchvision.models, name)
    vit = func(weights=weights, **kwargs)
    # modules = [m for m in vit.children() if m.__class__.__name__ != "Sequential"]
    # model = torch.nn.Sequential(*modules)
    vit.heads = torch.nn.Identity()
    # print(vit)
    return vit

def get_resnet(name:str="resnet18", weights=None, **kwargs) -> nn.Module:
    """
...
def print_model_parameter_sizes(model):
    ...
    for name, parameter in model.named_parameters():
        print(f"Layer: {name} | Size: {parameter.size()} | Total Params: {parameter.numel()}")
        keyname = name.split(".")[0]
        if not (keyname in params_dict.keys()):
            params_dict[keyname] = parameter.numel()
        else:
            params_dict[keyname] += parameter.numel()

    total_params = 0
    for name, value in params_dict.items():
        print("nets[{}].num_params: {}".format(name, value))
        total_params += value
...
class DropoutMLPWithPooling(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        super(DropoutMLPWithPooling, self).__init__()
        self.num_layers = num_layers
        self.layers = nn.ModuleList()
        self.dropout = nn.Dropout(0.2)
        
        assert int(hidden_dim**0.5)**2 == hidden_dim, "hidden_dim must be a perfect square"
        self.hidden_dim_side = int(hidden_dim**0.5) 
        self.layers.append(nn.Linear(input_dim, hidden_dim))
        curr_side_dim = self.hidden_dim_side
        for _ in range(1, num_layers):
            if _ % 2 == 1:  # Alternate between average and max pooling
                self.layers.append(nn.AvgPool2d(2, stride=2))
            else:
                self.layers.append(nn.MaxPool2d(2, stride=2))
            curr_side_dim //= 2
            self.layers.append(nn.Linear(curr_side_dim * curr_side_dim, curr_side_dim * curr_side_dim))
        self.layers.append(nn.Linear(curr_side_dim * curr_side_dim, output_dim))
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            if isinstance(layer, nn.Linear):
                ...
                x = x.view(-1, curr_side_dim, curr_side_dim)
